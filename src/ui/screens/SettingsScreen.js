 ...
 * SettingsScreen.js
 * -----------------------------------------------------------------
 * Owns its own local UI state (mute / volumes / infinite ad mode) and
...
export class SettingsScreen {
  constructor() {
    this.element = null;
    this.state = { muted: false, sfxVolume: 1, bgmVolume: 0.6, infiniteAdMode: false };
  }
  ...
  render() {
    ...
    this._adModeBtn = document.createElement('button');
    this._adModeBtn.type = 'button';
    this._adModeBtn.className = 'crystal-btn main-btn--gold';
    this._adModeBtn.addEventListener('click', () => {
      bus.emit(EVENTS.UI_CLICK);
      this.state.infiniteAdMode = !this.state.infiniteAdMode;
      this._renderAdModeBtn();
      bus.emit(EVENTS.SETTINGS_CHANGED, { ...this.state });
    });
  ...
  _renderAdModeBtn() {
    this._adModeBtn.textContent = this.state.infiniteAdMode ? i18n.t('infiniteAdModeOn') : i18n.t('infiniteAdModeOff');
  }
