...
class GameManager {
  ...
  /**
   * Ana Menüye Geçiş (3. Menü Geçişinde Reklam Gösterir)
   */
  showMainMenu() {
  ...
  /**
   * Oyun Seçim Ekranına Geçiş (3. Menü Geçişinde Reklam Gösterir)
   */
  showGameSelect() {
    adManager.handleMenuTransition(() => {
      this._teardownActiveGame();
  ...
  // -----------------------------------------------------------------
  // INTERNAL REKLAM VE OYUN AKIŞI
  // -----------------------------------------------------------------

  /**
   * CANLANMA KABUL EDİLDİĞİNDE (AdMob Ödüllü Reklam İzletir)
   */
  _handleReviveAccepted() {
    adManager.showReviveAd(
      () => {
        // Reklam Başarıyla İzlendi -> Oyuncuyu Canlandır
        if (this.activeGame && typeof this.activeGame.revive === 'function') {
          this.activeGame.revive();
        }
        this._reviveOfferPending = false;
      ...
      () => {
        // Reklam İptal Edildi / İzlense Bile Bitiş İşlemi Yapılabilir
        console.log('[GameManager] Canlanma reklamı tamamlanamadı.');
      }
    );
  }

  /**
   * "NO THANKS" / YENİDEN BAŞLAMA (2. Öldüğünde AdMob Geçiş Reklamı Gösterir)
   */
  _handleReviveDeclined() {
    adManager.handleDeathRestart(() => {
      // Skor kaydı
      this._saveHighScore(this.activeGameId, this.score);
      bus.emit(EVENTS.GAME_OVER, { gameId: this.activeGameId, score: this.score });
  ...
  _handleJokerRequested(jokerId) {
    adManager.showReviveAd(
      ...
      () => {
        console.log('[GameManager] Joker reklamı tamamlanamadı.');
      }
    );
  }
